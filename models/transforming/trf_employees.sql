...
with recursive managers
    ...
    as
      ...
      as
      (
      select ho.officeid, so.officecity, so.officecountry from {{ref('stg_satellite_offices')}} as so 
      inner join {{ref('stg_hub_Offices')}} as ho on so.officehashkey = ho.officehashkey
      )
 
  -- This is the "main select".
  select managers.indent, managers.employee_id, offices.officecity, offices.officecountry,
  managers.employee_title, managers.manager_id, managers.manager_title, managers.employee_name,managers.manager_name
